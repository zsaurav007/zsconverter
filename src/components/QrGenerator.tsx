...
export default function QrGenerator() {
  ...
  const handleDownload = () => {
    ...
    link.href = qrUrl
    link.download = `zs-qr-code-${Date.now()}.png`
    link.click()
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col md:flex-row h-[650px]">
      <div className="w-full md:w-80 h-full bg-slate-50 p-6 border-r border-slate-200 flex flex-col gap-6 overflow-y-auto pb-2">
        <h3 className="text-xs font-bold text-slate-800 uppercase tracking-widest border-b pb-4 flex items-center gap-2 flex-shrink-0"><Settings2 className="w-4 h-4"/> QR Configuration</h3>
        
        <div className="space-y-3 flex-shrink-0">
          <label className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest"><Link2 className="w-3 h-3"/> Content URL or Text</label>
          <textarea 
            value={text} 
        ...
        <div className="space-y-3 flex-shrink-0 pt-4 border-t border-slate-200">
      ...
      <div className="flex-1 p-6 md:p-8 bg-slate-100 flex flex-col items-center justify-center h-full">
        {qrUrl ? (
          <div className="bg-white p-6 rounded-2xl shadow-xl flex flex-col items-center gap-4 border border-slate-200">
            <img src={qrUrl} alt="Generated QR Code" className="w-64 h-64 object-contain" />
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate max-w-[250px]">{text || 'Empty'}</p>
          </div>
        ) : (
